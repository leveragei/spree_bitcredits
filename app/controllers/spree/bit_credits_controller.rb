  ...
  class BitCreditsController < StoreController
    def save_balance
      ...
      if session[:bitcredits_old_balance].blank?
        session[:bitcredits_old_balance] = balance.to_f unless balance.nil?
      end

      if bitc != session[:bitcredits_old_balance]
        session[:bitcredits_new_balance] = balance
      end

      session[:bitcredits_cockie] = bitc unless bitc.nil?
      render js: "Saved"
    end
